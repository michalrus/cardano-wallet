...
numberOfTransactionsInBlock = \case 
    ...
    where 
        transactions (ShelleyBlock (SL.Block (SL.BHeader header _) (SL.TxSeq txs')) _) 
            = (length txs', (fromBlockNo $ SL.bheaderBlockNo header, SL.bheaderSlotNo header))
        transactionsAlonzo (ShelleyBlock (SL.Block (SL.BHeader header _) (Alonzo.TxSeq txs')) _) 
            = (length txs' , (fromBlockNo $ SL.bheaderBlockNo header, SL.bheaderSlotNo header))
        transactionsByron blk = (, (fromBlockNo $ O.blockNo blk, O.blockSlot blk)) $
            case byronBlockRaw blk of
            ABOBBlock blk'  -> length $ fromTxAux <$> unTxPayload (blockTxPayload blk')
            ABOBBoundary _ -> 0
